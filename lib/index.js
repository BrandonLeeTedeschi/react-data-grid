...
function getTopLeftBoundSelectedRange(selectedRange) {
	return {
...
function Cell({ column, colSpan, isCellSelected, isDraggedOver, selectedCellIdx, row: row$1, rowIdx, className, onMouseDown, onMouseUp, onMouseEnter, onCellMouseDown, onClick, onCellClick, onDoubleClick, onCellDoubleClick, onContextMenu, onCellContextMenu, onRowChange, selectCell, style, rangeSelectionMode, selectionMouseDown, selectionMouseUp, selectionMouseEnter,...props }) {
	const { tabIndex, childTabIndex, onFocus } = useRovingTabIndex(isCellSelected);
	const rootSelectedCellClassname = "rq910m07-0-0-beta-56";
...
//#endregion
//#region src/Row.tsx
function Row({ className, rowIdx, gridRowStart, selectedCellIdx, selectedCellsRange, isRowSelectionDisabled, isRowSelected, draggedOverCellIdx, lastFrozenColumnIndex, row: row$1, viewportColumns, selectedCellEditor, onCellMouseDown, onCellClick, onCellDoubleClick, onCellContextMenu, selectionMouseDown, selectionMouseUp, selectionMouseEnter, rowClass, onRowChange, selectCell, style, rangeSelectionMode,...props }) {
	...
	*/
	...
	function handleCellCopy(event) {
		...
		if (enableRangeSelection) {
			...
			const sourceColumnKeys = columns.slice(cellsRange.startColumnIdx, cellsRange.endColumnIdx + 1).map((c) => c.key);
			onMultiCopy?.({
				cellsRange,
				sourceRows,
				sourceColumnKeys
	...
	*/
	...
	function selectCell(position, options) {
		...
		if (options?.enableEditor && isCellEditable(position)) {
			...
			if (endRowIdx - startRowIdx === 0 && endColumnIdx - startColumnIdx === 0) scrollIntoView(getCellToScroll(gridRef.current));
		...
		for (let viewportRowIdx = startRowIdx; viewportRowIdx <= endRowIdx; viewportRowIdx++) {
			...
			rowElements.push(renderRow(key, {
				...
				selectionMouseDown: (args, event) => {
					...
					const { rowIdx: rowIdx$1, column, selectCell: selectCell$1 } = args;
					if (event.shiftKey) setSelectedRange({
						...selectedRange,
						endRowIdx: rowIdx$1,
				...
				selectionMouseEnter: ({ rowIdx: rowIdx$1, column }) => {
